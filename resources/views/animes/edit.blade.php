...
<div class="content-box big-box">
  ...
  <form id="form_anime" class="" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
      <label>Name</label>
      <input type="text" class="form-control" name="name" value="{{ $anime->name }}">
    </div>

    <div class="form-group">
      <label>Synopsis</label>
      <textarea class="form-control" rows="5" name="synopsis">{{ $anime->synopsis }}</textarea>
    </div>
    ...
    <div class="form-group">
      ...
      <select class="form-control" name="season" id="season">
        ...
        <option value= {{$season->id}} {{ ($season->id == $anime->season_id) ? 'selected' : '' }}>{{$season->name}}</option>
        @endforeach
      </select>
    </div>
    ...
    <div class="form-group">
      ...
      <div class="checkbox checkbox-primary">
        <label>
          <input type="checkbox" name="anime_genre[]" value= {{ $anime_genre->id }} {{ in_array($anime_genre->id, $anime_genres_anime) ? 'checked' : ''}}>
          <i></i>
          {{ $anime_genre->name }}
        </label>
      </div>
    ...
    <div class="fileinput fileinput-new input-group" data-provides="fileinput">
      ...
      <div class="form-control" data-trigger="fileinput"><span class="fileinput-filename" value="{{ $anime->img }}">{{ $anime->img }}</span>
      </div>
